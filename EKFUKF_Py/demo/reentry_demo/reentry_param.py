...
true_m0 = np.array([6500.4, 349.14, -1.8093, -6.7967, 0.6932])[:,None]

true_P0 = np.diag([1e-6, 1e-6, 1e-6, 1e-6, 0])

true_Qc = np.diag([2.4064e-5, 2.4064e-5, 0]) / 0.1
...
m0 = np.array([6500.4, 349.14, -1.8093, -6.7967, 0])[:,None]

P0 = np.diag([1e-6, 1e-6, 1e-6, 1e-6, 1])

Qc = np.diag([2.4064e-5, 2.4064e-5, 1e-6]) / 0.1
